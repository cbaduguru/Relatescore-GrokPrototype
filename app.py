...
# Implements Fixes-3.docx recommendations:
# - Home screen (after Create Profile -> Continue OR Log In) has exactly 3 buttons:
#   Create Invite, Enter Invite, Withdraw and Reset
# - Removes "Enter Invite Code" phrasing from Home (uses "Enter Invite" instead)
# ------------------------------------------------------------
st.set_page_config(page_title="RelateScore™", page_icon="✅", layout="centered")
st.set_option("client.showErrorDetails", True)  # helpful on Streamlit Cloud
...
def init_state():
    defaults = {
        ...
        # Invite flow
        "invite_code": None,     # generated by Create Invite
        "partner_code": "",      # typed by user
...
def entry_page():
    ...
    # Entry screen keeps this for direct join
...
def home_page():
    # Home MUST have exactly 3 buttons per Fixes-3.docx
    display_logo()
    st.header("Home")
...
def enter_invite_page():
    ...
    if st.session_state.invite_code is None:
        st.info("No active invite is available in this app session. Ask the sender to click Home → Create Invite and share the code.")
    ...
    with c2:
        if st.button("Continue", key="enter_invite_continue"):
            if st.session_state.invite_code and st.session_state.partner_code == st.session_state.invite_code:
